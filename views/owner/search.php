...
$q = trim($_GET['q'] ?? '');
$customers = $q
    ? dbSelect("SELECT * FROM customers WHERE name LIKE ? OR customer_id LIKE ? OR phone LIKE ? ORDER BY name", ["%$q%","%$q%","%$q%"])
    : dbSelect("SELECT * FROM customers ORDER BY id DESC");
layoutStart('Search','search');
?>
<h1 class="page-title">SEARCH</h1>
<form method="GET" action="/owner/search" style="margin-bottom:20px;">
